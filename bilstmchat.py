...
data=data[['Label','Class']]
data = data.dropna()

text_data=data['Label']
text_classes=data['Class']
...
data['Label'] = text_data
data['Class'] = text_classes
...
train_p=data[['tweet_id','Class','dum_val',]].pivot_table(index='tweet_id',columns='Class')
train_p=pd.DataFrame(train_p.to_records())
train_p.columns=['tweet_id','Very Unsatisfied','Unsatisfied','Neutral','Satisfied','Very Satisfied']
train_new=pd.merge(data,train_p,on='tweet_id',how='left')
train_new=train_new.drop(['Class','dum_val'],axis=1)
train_2=train_new[["Very Unsatisfied","Unsatisfied","Neutral","Satisfied","Very Satisfied"]].fillna(0)
list_classes = ['Very Unsatisfied','Unsatisfied','Neutral','Satisfied','Very Satisfied']
