...
data_train = pd.read_csv("./hcr/train/orig/hcr-train.csv")
print("Train File Read")

data_test = pd.read_csv("./hcr/test/orig/hcr-test.csv")
print("Test File Read")

data_val = pd.read_csv("./hcr/dev/orig/hcr-dev.csv")
print("validation File Read")


# Train Text Data
train_text_data = data_train['content']
train_text_classes = data_train['sentiment']


# Test Text Data
test_text_data = data_test['content']
test_text_classes = data_test['sentiment']


# Train Text Data
val_text_data = data_val['content']
val_text_classes = data_val['sentiment']

#print(val_text_data[0])
#exit()

'''
# label encode the target variable 
encoder = preprocessing.LabelEncoder()
train_y = encoder.fit_transform(train_text_classes)
valid_y = encoder.fit_transform(val_text_classes)
test_y = encoder.fit_transform(test_text_classes)

train_x = train_text_data
valid_x = val_text_data
test_x = test_text_data
'''
 
new_data = pd.concat([data_train, data_val, data_test], ignore_index=True)
new_data = new_data[['content','sentiment']]
...
text_data = new_data['content']
text_classes = new_data['sentiment']
...
# Removing Empty Rows
#text_classes = text_classes.dropna()
#print(text_data)

#print(np.unique(text_classes))
#exit()
